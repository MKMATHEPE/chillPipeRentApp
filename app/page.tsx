...
export default function Store(){
  ...
  return <main className="store-app">
    ...
    <div className="store-layout"><section className="catalog"><div className="catalog-heading"><div><p className="eyebrow">Order your way</p><h2>{category==='All'?'Everything you need':category}</h2></div><span>{visible.length} items</span></div><div className="product-grid">{visible.map(item=>{const qty=cart[item.id]||0;return <article className="app-product" key={item.id}><div className={`product-visual ${item.category.toLowerCase()}`}>{item.category==='Rentals'?<img src="/chill-pipe-logo.png" alt=""/>:item.category==='Flavours'?<Flame size={30}/>:<Package size={30}/>} {item.badge&&<span>{item.badge}</span>}</div><div className="product-copy"><div><h3>{item.name}</h3><strong>{money(item.price)}</strong></div><p>{item.description}</p></div><div className="product-action">{qty===0?<button type="button" onClick={()=>adjust(item.id,1)}>Add <Plus size={15}/></button>:<div className="mini-stepper"><button type="button" onClick={()=>adjust(item.id,-1)}><Minus size={14}/></button><span>{qty}</span><button type="button" onClick={()=>adjust(item.id,1)}><Plus size={14}/></button></div>}</div></article>})}</div></section>
    ...
    <button className="mobile-cart" onClick={checkout} disabled={!count}><span><ShoppingBag size={17}/> View cart · {count}</span><strong>{money(total)}</strong></button>
  </main>
}
